...
void LaserManager:: setup (int width, int height) {
	...
	parameters.add(ignoreMask.set("ignore mask", false));
...
//	
	if(testPattern==1) {
		...
		for(float x =0; x<=1; x+=0.2) {
			...
			addLaserLineEased(ofPoint(ofMap(x,0,1,maskRectangle.getLeft()+0.001,maskRectangle.getRight()), maskRectangle.getTop()),ofPoint(ofMap(x,0,1,maskRectangle.getLeft()+0.001,maskRectangle.getRight()), maskRectangle.getBottom()+100), ofColor::red );
			
		}
		for(float y = 0; y<=1; y+=0.2) {

			addLaserLineEased(ofPoint(maskRectangle.getLeft(),ofMap(y,0,1,maskRectangle.getTop()+0.001,maskRectangle.getBottom())),
							  ofPoint(maskRectangle.getRight(), ofMap(y,0,1,maskRectangle.getTop()+0.001,maskRectangle.getBottom())), ofColor::white );
	...
	while(ildaPoints.size()<minPoints) {
		LaserCircle blank(startPosition + ofPoint(0,10), black, 50);
...
//	}
	...
	while (currentAngle<=maxAngle + 360) {
		...
		float brightness = 1;
		if(currentAngle>maxAngle+180) {
			brightness=ofMap(currentAngle, maxAngle+180, maxAngle+360, 1,0);
			
		}
		addIldaPoint(pos, spiral.colour, spiral.intensity*brightness);
...
void LaserManager::addIldaPoint(ofPoint p, ofFloatColor c, float pointIntensity, bool useCalibration){
	...
	if((!ignoreMask) && (p.x<maskRectangle.getLeft() ||
	   ...
	   p.y>maskRectangle.getBottom())) {
		...
		if(offScreen){
			...
			ofPoint target = lastClampedOffScreenPoint; // shouldn't this be p? TODO
...
void LaserManager::addPoint(ofPoint p, ofFloatColor c, bool useCalibration) {
	...
	ofPoint warpedpoint = warp.getWarpedPoint(p);
		...
		if(!isPointValid(warpedpoint)) return;
...
bool LaserManager::isPointValid(ofPoint& p) {
	
	if((p.x>=kIldaMinPoint)  && (p.x<=kIldaMaxPoint) && (p.y>=kIldaMinPoint)  && (p.y<=kIldaMaxPoint)) return true;
	else {
		ofLog(OF_LOG_ERROR, "invalid ilda point");
	}
