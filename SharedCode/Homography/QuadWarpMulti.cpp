...
//
//  The QuadWarpMulti object is used to calculate homography between two quads.
...
QuadWarpMulti :: QuadWarpMulti (string saveLabel) {
	...
	ofAddListener(ofEvents().mousePressed, this, &QuadWarpMulti::mousePressed);
	ofAddListener(ofEvents().mouseDragged, this, &QuadWarpMulti::mouseDragged);
	ofAddListener(ofEvents().mouseReleased, this, &QuadWarpMulti::mouseReleased);


}

QuadWarpMulti :: ~QuadWarpMulti() {
	
	ofRemoveListener(ofEvents().mousePressed, this, &QuadWarpMulti::mousePressed);
	ofRemoveListener(ofEvents().mouseDragged, this, &QuadWarpMulti::mouseDragged);
	ofRemoveListener(ofEvents().mouseReleased, this, &QuadWarpMulti::mouseReleased);

}

void QuadWarpMulti :: draw() {
	draw(ofGetKeyPressed(OF_KEY_SHIFT));

}

void QuadWarpMulti :: draw(bool lockAxis) {
...
void QuadWarpMulti::clampPoints(){
    if(useBoundsRect) {
        for(int i=0;i<dstPoints.size();i++){
            ofVec3f& point = dstPoints[i];
...
void QuadWarpMulti::drawMarker(ofVec3f& point, const ofColor & color, float radius){
...
void QuadWarpMulti::setOffset(float x,float y){
    offset.x = x;
    offset.y = y;
}

void QuadWarpMulti ::setDstPoint(int index, ofVec3f point){
...
void QuadWarpMulti ::setSrcPoint(int index, ofVec3f point){
...
ofVec3f QuadWarpMulti::getCenter(){
...
void QuadWarpMulti ::apply(ofRectangle sourceRect){
...
void QuadWarpMulti::updateHomography() {
...
ofVec3f QuadWarpMulti::getWarpedPoint(ofVec3f point){
...
//bool QuadWarpMulti::rayInterectionWithLine(ofVec3f p1, ofVec3f r1, ofVec3f p3, ofVec3f p4, ofVec3f& intersection ){
//    
//    ofVec3f p2 = p1 + r1;
//    ofVec3f u = p4-p3;
//    ofVec3f v = p2-p1;
...
//}
//
//bool QuadWarpMulti::rayIntersectionWithBezier(ofVec3f lp1, ofVec3f r1,ofVec3f bp1,ofVec3f bp2,ofVec3f cp1, ofVec3f cp2, ofVec3f& closestIntersection){
//    
//    ofVec3f lp2 = lp1+r1;
//    float A = lp2.y - lp1.y;
//    float B = lp1.x - lp2.x;
...
//}
//
//vector <float> QuadWarpMulti::bezierCoeffs(float P0,float P1,float P2,float P3)
//{
//	vector <float> Z(4);
//	Z[0] = -P0 + 3*P1 + -3*P2 + P3;
//    Z[1] = 3*P0 - 6*P1 + 3*P2;
...
//  */
// vector<float> QuadWarpMulti::cubicRoots(float a, float b, float c, float d){
//    float A = a/b;
//    float B = b/c;
//    float C = c/d;
//    float Q = ( 3*B - A*A ) / 9;
...
ofVec3f QuadWarpMulti::getUnWarpedPoint(ofVec3f point){
...
void QuadWarpMulti :: mousePressed(ofMouseEventArgs &e) {
...
void QuadWarpMulti :: mouseDragged(ofMouseEventArgs &e) {
	if(!visible) return;
...
void QuadWarpMulti :: mouseReleased(ofMouseEventArgs &e) {
	if(!visible) return;
...
bool QuadWarpMulti::loadSettings() {
	  
	string filename = "warpdata/"+label+".xml";
	ofxXmlSettings xml;
	if(!xml.loadFile(filename)) {
		ofLog(OF_LOG_ERROR, "QuadWarpMulti::loadSettings - file not found : "+filename);
		return false;
...
void QuadWarpMulti::saveSettings() {
...
std::ostream& operator<< (std::ostream& stream, const QuadWarpMulti& warp){
...
std::istream& operator>> (std::istream& stream, QuadWarpMulti& warp){
    float component;
