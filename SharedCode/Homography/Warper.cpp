...
void Warper :: updateWarpedImage(ofPixelsRef sourcePixels) {
	...
	if(sourcePixels.getImageType()!=warpedImage.getPixelsRef().getImageType()){
		...
		srcImage.allocate(dstWidth* dstPreviewScale, dstHeight* dstPreviewScale, sourcePixels.getImageType());
	}
	vector<Point2f> srcPoints, dstPoints;
	...
	warpPerspective(sourcePixels, warpedImage, previewHomography, CV_INTER_NN);
//	warpPerspective(sourcePixels, warpedImage, homography, CV_INTER_NN);
	warpedImage.update();
	
	copy(sourcePixels, srcImage);
	srcImage.update();
...
void Warper :: draw(bool showWarped) {
	...
	if(showWarped) warpedImage.draw(0,0, dstWidth, dstHeight);
	else srcImage.draw(0,0, dstWidth, dstHeight);
